...
| A16 | Review finding: `computeBackoffDelay` capped a server `Retry-After` at `maxDelayMs` (a 120 s wait became 30 s), which retries earlier than instructed. | Fixed: `maxDelayMs` bounds only the local formula; `Retry-After` is honoured in full (seconds or HTTP date). `withRetry` takes an operational maximum (`HUBSPOT_MAX_RETRY_WAIT_MS`, default 120 s): above it the request is not retried and the error carries `retryWaitExceeded: true` and `retryAfterMs`. Max attempts, jitter and the uncertain-POST protection are unchanged. Regression tests in `test/handleHubSpotErrors.test.js`. |
| A17 | Review finding: a deal saved by the sync whose association failed was counted as created/updated with `association.status = 'failed'`, the script exited 0, and 401/403 raised while associating did not abort. | Fixed: `applyDealResult` puts such entries in `summary.partial` with the deal id and `dealStatus` (never counted twice, never as "deal not created"); `hasSyncFailures` makes the scripts exit 1 for failed, partial or aborted runs; `associateDealWithContact` re-throws 401/403 so the run aborts; `skipped (CONTACT_NOT_FOUND)` remains a warning (data condition) with exit 0. Idempotent: a re-run finds the deal by `external_id` and re-attempts only the association. Tests in `test/syncHelpers.test.js`. |
| A18 | Review finding: association reads used only the first page of `GET .../associations/{to}`. | Fixed: `getContactDealAssociations` / `getDealContactAssociations` walk every page with `paginateAll` (`limit` 500, the documented default and maximum for this endpoint); `associateContactToDeal` uses `findAcrossPages`, which stops at the page where the deal is found. Tests in `test/pagination.test.js` cover an item on a later page. |
| A19 | Review finding: `computeChangedProperties` compared `closedate` as text, so equivalent instants in different notations (`...:00Z` vs `...:00.000Z`) were reported as changes. | Fixed: date-aware comparison only for declared date properties (`closedate`), by instant; empty/null means clear (sent as `""`); unparseable values fall back to text; `closedate` is validated and normalized to canonical ISO 8601 before the sync. Tests in `test/syncHelpers.test.js` and `test/validateHubSpotPayload.test.js`. |
