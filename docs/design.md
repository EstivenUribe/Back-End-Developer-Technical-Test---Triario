...
Stage 1 deliverable: analysis and design. No business logic is implemented yet.
Source of acceptance criteria: `Back End Developer Technical Test.pdf` (Triario).
Official HubSpot documentation was consulted on 2026-09-17; URLs are listed in section 6.
...
| C12 | Pagination handled per docs | `src/utils/pagination.js` → `paginateAll(fetchPage)` loops on `paging.next.after`, `limit=100` (max allowed on list endpoints) | Run `get-contact-names.js` on a portal with > 100 contacts (a seed script creates them) and compare the count with the UI |
...
| S2 | `syncContactsWithHubSpot` — sync local JSON, idempotent create/update | `src/services/syncService.js` → `syncContactsWithHubSpot(contacts)`; natural key = `email`; per record: `contactRepository.getByEmail` (`idProperty=email`) → 404 ⇒ create, otherwise PATCH only when properties differ; source `src/data/contacts.json` | Run `node src/examples/sync-contacts.js` twice; first run creates N, second run reports 0 created / N unchanged; UI shows no duplicates |
| S3 | `syncDealsWithHubSpot` — same for deals | `syncService.syncDealsWithHubSpot(deals)`; natural key = custom unique deal property `external_id` (created once through the Properties API with `hasUniqueValue: true`), lookup with `idProperty=external_id`; source `src/data/deals.json`; optionally associates each deal with a contact by email | Run `node src/examples/sync-deals.js` twice; second run creates nothing; UI shows no duplicates |
| S4 | Sync must handle errors per record without aborting the batch | `syncService` collects `{ created, updated, unchanged, failed[] }` and returns a summary; each record wrapped in `try/catch` | Run with one intentionally invalid record in the JSON → summary shows 1 failed and the rest processed |
...
| A11 | The Deals reference documents `idProperty` explicitly only for batch read (`POST /crm/v3/objects/deals/batch/read` with `idProperty` in the body); the Contacts reference documents it on the single-record `GET`/`PATCH` as well. | Try `GET /crm/v3/objects/deals/{value}?idProperty=external_id` first (same pattern as contacts by email) and verify with a real call in stage 5; if the portal rejects it, use the documented batch read with one input. Either way the lookup is exact. |
...
| 7 | Sync services with local JSON (`syncContactsWithHubSpot`, `syncDealsWithHubSpot`) | Two consecutive runs: second run creates nothing |
| 8 | README, final review against this matrix, submission checklist | Every row in section 2 has been executed at least once |
